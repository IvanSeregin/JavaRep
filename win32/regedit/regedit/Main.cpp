...
HWND regeditTreeView; //контрол дл€ отображени€ дерева реестра
HWND regeditListView; // контрол дл€ отображени€ значений выбранной ветки
HMENU mainMenu; //меню приложени€
HTREEITEM *root; //корневой элемент дл€ TreeView
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		case WM_COMMAND: 
		{
			if (LOWORD(wParam) == FILE_EXIT) //нажат пункт меню ¬ыход
			{
				SendMessage(hWnd, WM_CLOSE, 0, 0);
			}
			if (LOWORD(wParam) == FILE_DUMP) //нажат пункт меню —оздать дамп
			{
				dumpRegistry(); //вызов функции создани€ дампа
			}
			break;
		}
		case WM_CLOSE:
		...
		case WM_NOTIFY:
		{
			...
			{
			case REG_TREE_VIEW: // событи€ св€занные с контролом TreeView
				if (((LPNMHDR)lParam)->code == TVN_SELCHANGED)//событие на выделение пункта в списке
				{
					...
					/*≈сли выделена строка отлична€ от корн€
					то находим полный путь в реестре до этого каталога
					и перечисл€ем все ключи и их значени€ в контроле ListView
					*/
					if (hClicked != *root) 
					{
